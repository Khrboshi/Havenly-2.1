  ...
  for (const userId of staleUserIds) {
    ...
    try {
      ...
      if (result.ok) {
        ...
        const failure = result as { ok: false; reason: "no_data" | "groq_failed" | "db_error" };
        if (failure.reason !== "no_data") errors++;
        console.log(`[cron/weekly-summaries] ⚠ ${userId} — skipped (${failure.reason})`);
      }
    } catch (err) {
      errors++;
