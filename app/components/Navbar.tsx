...
export default function Navbar({ user: initialUser }: { user: User | null }) {
  ...
  // Sync auth state in real time — fixes stale navbar after logout
  useEffect(() => {
      ...
      <nav className="mx-auto flex h-16 max-w-5xl items-center justify-between px-4">
        <Link href={loggedIn ? "/dashboard" : "/"} className="flex items-center gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-500/10 ring-1 ring-emerald-400/40">
            <span className="text-sm font-semibold tracking-wide text-emerald-300">H</span>
          </div>
        ...
        <div className="flex items-center gap-4">
          {loggedIn ? (
            <>
              <Link href="/dashboard" className="text-sm text-slate-300 hover:text-emerald-200">
                Dashboard
              </Link>
              <Link href="/journal" className="text-sm text-slate-300 hover:text-emerald-200">
                Journal
              </Link>
              <Link href="/settings" className="text-sm text-slate-300 hover:text-emerald-200">
                Settings
              </Link>
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {menuOpen && (
                  <div className="absolute right-0 mt-2 w-52 rounded-xl border border-slate-800 bg-slate-950 p-1 text-sm shadow-xl">
                    <Link href="/settings" onClick={() => setMenuOpen(false)} className="block rounded-lg px-3 py-2 text-slate-200 hover:bg-slate-800">
                      Account & settings
                    </Link>
                    <Link href="/logout" onClick={() => setMenuOpen(false)} className="block rounded-lg px-3 py-2 text-rose-300 hover:bg-rose-900/40">
                      Logout
                    </Link>
                  </div>
                )}
            ...
            <>
              ...
              <Link href="/magic-login" className="rounded-full bg-emerald-400 px-4 py-2 text-sm font-medium text-slate-950 hover:bg-emerald-300">
                Get started
              </Link>
            </>
          )}
